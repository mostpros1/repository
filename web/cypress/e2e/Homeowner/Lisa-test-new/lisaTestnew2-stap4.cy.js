...
describe('Testing "Register as specialist"', () => {
 ...
 //    it(`type in input - iteration ${i+1}`, () =>
    ...
    cy.lisaTestBegin();
     ...
     // TEST 2
      ...
      for (let i = 1; i < 6; i++) {
        ...
        //   if ($body.find('.login-indicator').length === 0) { // Aannemend dat '.login-indicator' zichtbaar is als de gebruiker is ingelogd
        //     // Gebruiker is niet ingelogd, voer inputcheck uit
        //     //performInputCheck();
        //   }
        // });
        cy.goForward();
    ...
    cy.get('.contact-button').should('be.visible').click();  // Corrected the selector by closing the quotes properly
    cy.get('[class="form-btn back"]').click();
