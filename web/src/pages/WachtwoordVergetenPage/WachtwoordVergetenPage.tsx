    ...
    const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        if (!codeSent) {
            try {
                ...
                setCodeSent(true);
                alert("Een verificatiecode is naar uw e-mail verzonden. Controleer uw inbox.");
            } catch (error) {
                alert("Fout bij het verzenden van de verificatiecode. Probeer het later opnieuw.");
            }
        } else {
            ...
            try {
                await Auth.forgotPasswordSubmit(fields.email, code, fields.newPassword);
                alert("Uw wachtwoord is succesvol gewijzigd. U kunt nu inloggen met het nieuwe wachtwoord.");
                window.location.href = "http://localhost:5173/login";
            } catch (error) {
                alert("Er is een fout opgetreden bij het wijzigen van uw wachtwoord. Probeer het later opnieuw.");
            }
        }
    };
    ...
    return (
        <div className={codeSent ? "resetpassword-container codesent" : "resetpassword-container"}>
            <form className={codeSent ? "resetpassword-card codesent" : "resetpassword-card"} onSubmit={onSubmit}>
                ...
                <button className="resetpassword-card_button" type="submit">Bevestigen</button>
                { codeSent && <a className="confirmemail-card_newCode" onClick={() => setCodeSent(false)}>Nieuwe code versturen</a>}
            </form>
        </div>
    );
};
