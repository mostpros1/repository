...
export default function SearchChoreForm({
  beroep,
  email,
  postCode,
  stad,
  updateFields,
}: SearchChoreFormProps) {
  ...
  return (
    <>
      <div className="search_chore_text_con">
        <h1>Zoek uw klus</h1>
        <p>
          Klussen worden gezocht in alle sectoren en door heel Nederland. Laat
          ons weten waar u wilt werken, dan helpen wij u bij het vinden van
          passende klussen.
          <br />
          Als u een eenmansbedrijf hebt, voer uw eigen gegevens in. Als u meer
          mensen hebt die bij uw bedrijf werken, voer de gegevens van het
          contactpersoon in (bedrijfseigenaar of een filiaalmanager).
        </p>
      </div>
      <div className="search_chore_form">
        <label>Uw hoofdberoep</label>
        {/* Placeholder toevoegen voor specialisatie */}
        <input
          type="text"
          required
          placeholder="Uw specialisatie"
        ...
        {!isValidBeroep && (
          <p className="error-message">
            Voer alstublieft een geldige beroep in
          </p>
        )}
        <label>Email:</label>
        <input
        ...
        {!isValidPostcode && (
          <p className="error-message">
            Voer alstublieft een geldige postcode in (bijv. 1234AB)
          </p>
        )}
        {!isValidStad && (
          <p className="error-message">
            Voer alstublieft een geldige stad in (bijv. Amsterdam)
          </p>
        )}
        <p className="form_login">
          Al een account?{" "}
